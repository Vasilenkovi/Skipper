...
class SlotService {
    ...
    suspend fun getSlotsForExpert(userIdFromToken: String): List<SlotResponse> =
        dbQuery {
            try {
    ...
    suspend fun bookSlot(
        ...
        dbQuery {
            try {
                ...
                val slotRow =
                    Slots.select { Slots.id eq slotUuid }.singleOrNull()
                ...
                val expertProfile =
                    ExpertProfiles
                        .select { ExpertProfiles.id eq slotRow[Slots.expertId] }
                        .singleOrNull()
    ...
    suspend fun acceptSlotRequest(
        ...
        dbQuery {
            try {
                ...
                val expertProfile =
                    ExpertProfiles.select { ExpertProfiles.userId eq userUuid }.singleOrNull()
    ...
    suspend fun getAvailableSlotsForMentor(mentorUserId: String): List<SlotResponse> =
        dbQuery {
            try {
                val expertProfileRow =
                    ExpertProfiles
    ...
    suspend fun confirmPayment(slotId: String): String? =
        dbQuery {
            try {
                val slotUuid = UUID.fromString(slotId)
    ...
    suspend fun getSlotsForMentee(userIdFromToken: String): List<MenteeSlotResponse> =
        dbQuery {
            try {
                val userUuid = UUID.fromString(userIdFromToken)
    ...
    suspend fun cancelSlotByMentee(
        userIdFromToken: String,
        slotId: String,
    ): String? =
        dbQuery {
